...
max_iters = 5_000
eval_interval = 500
learning_rate = 1e-3 # self-attention block canot tolerate high lr
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        ## the same dim of the embedding. But now since we have 4 heads and we are going to concatenate the outputs we set the head_size to 8 = 32/4
        ## so what i think is that we are giving each head 8 'different' (i think) dimensions to play with and 'give' meaning to those directions.
        ## so the entire 32 dim representation of a token is more rich (complex, i.e. has (can hold) more meaning).
        ## Andrej says that this is like a group convolution, because istead of having a big convolution we do one of groups.
